...
//initializing mines position matrix
var minesp1 = [];
for(var i=0; i<6; i++) {
  minesp1[i] = [];
  for(var j=0; j<8; j++) {
...
function main() {
  ...
  //grass plane
 {
  ...
  class GPUPickHelper {
    ...
    //patch click
    click(cssPosition, scene, camera) {
      const {pickingTexture, pixelBuffer} = this;
    ...
    pickmines(cssPosition, scene, camera) {
      const {pickingTexture, pixelBuffer} = this;
    ...
    clickmines(cssPosition, scene, camera) {
      const {pickingTexture, pixelBuffer} = this;
      ...
      if(id!=0 && minesp2[Math.floor((id-2)/8)][(id-2)%8]==0 && nminesp2<5){
        ...
        if (intersectedObject) {
          ...
          setMine(Math.floor((id-2)/8)-3, (id-2)%8-4, nminesp2);
          nminesp2++;
          document.getElementById('mines').innerHTML="PC mines positioned: "+nminesp2+"/5";
          if(nminesp2==5){
            setTimeout(function(){ 
...
//first chocobo
  ...
  clearPickPosition();
    ...
    function moveCamera(n){
      if (n==1){
        cameramoving=1;
        tween4[0] = new TWEEN.Tween(camera.position, groupCamera1).to({x:0, y:20, z:-58}, 1000).start().onUpdate(function(){camera.lookAt(0,-10, 5);}).onComplete(function(){cameramoving=0; tween4=[]; groupCamera1.removeAll(); if(turn==4){choosepatch(clickedp2);}});
        setTimeout(function(){
          document.getElementById('turn').innerHTML="PC's turn";
        },1000);
      }
      else if (n==2){
        cameramoving=2;
    ...
    function setMine(i, j, n){
      ...
      gltfLoader.load('../models/rocks/scene.gltf', (gltf) => {
        ...
        // from root and below
          ...
          //Reposition to 0,halfY,0
          root.position.copy(boxCenter).multiplyScalar(-1);
          root.position.set(step*j+step/2,0.1,step*i+step+step/2-28);
      ...
      while(!done){
        ...
        if(patchmat[randomx][randomy]==0){
          if(minesp2[randomx][randomy]==1){
            ...
            plantGysahl(randomx-3, randomy-4, -28);
            turn=3;
          }
        }
      }
  ...
  function render(time) {
    ...
    if(document.getElementById('popup_content_wrap').style.display=="none" && document.getElementById('done').style.display=='none' && exploded==0 && animating==0 && cameramoving==0){
      if(turn==3){
        pickHelperp1.pick(pickPosition, pickingScenep1, camera);
      }
    }
      ...
      if(turn==2 && nminesp2<5){
        pickHelperp2.pickmines(pickPosition, pickingScenep2, camera);
      }
    }
  ...
  function click(){
    if(document.getElementById('popup_content_wrap').style.display=="none"&& exploded==0 && animating==0 && cameramoving==0){
      if(turn==2){
        pickHelperp2.clickmines(pickPosition, pickingScenep2, camera);
      }
      if(turn==3){
        pickHelperp1.click(pickPosition, pickingScenep1, camera);
      }
    }
  }
...
function initializeMines(posmines){
  ...
  while (nminesp1<5){
    ...
    if(posmines[randomx][randomy]==0){
      posmines[randomx][randomy]=1;
      nminesp1++;
    }
  }
}
